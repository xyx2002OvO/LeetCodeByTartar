...
| 2026-06-27 | 19 | 滑动窗口：无重复 | 0003 无重复字符的最长子串 [M] | 0904 水果成篮 [M] | 已完成 | 340/159 为会员题，改用 904 替代；3 维护无重复窗口，904 维护最多两类元素窗口。 |
| 2026-06-28 | 20 | 滑动窗口：字符频次 | 0438 找到字符串中所有字母异位词 [M] | 0567 字符串的排列 [M] | 已完成 | 补刷；两题共用固定长度频次窗口，438 记录所有起点，567 判断是否存在。 |
| 2026-06-29 | 21 | 滑动窗口：最小覆盖 | 0076 最小覆盖子串 [H] | 0076 最小覆盖子串 [R2][H] | 已完成 | 补刷；need/window + valid 模板，窗口合法后收缩并更新最短答案。 |
| 2026-06-30 | 22 | 滑动窗口：预算模型 | 0424 替换后的最长重复字符 [M] | 1004 最大连续 1 的个数 III [M] | 已完成 | 424 用历史 maxCnt 做单调上界优化；1004 只按 zeroCnt>k 收缩，允许窗口为空。 |
| 2026-07-01 | 23 | 字符串：回文 | 0005 最长回文子串 [M] | 0647 回文子串 [M] | 已完成 | 两题共用区间 DP：dp[i][j] 表示子串是否回文；5 更新最长区间，647 统计所有 true 状态。 |
